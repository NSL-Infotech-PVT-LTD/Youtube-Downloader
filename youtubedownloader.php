...
//        echo '<pre>';
//        print_r(json_decode($data['player_response']));
//        die;
        ...
        $result['views'] = isset(json_decode($data['player_response'])->videoDetails->viewCount) ? json_decode($data['player_response'])->videoDetails->viewCount : 0;
        $result['channel_id'] = isset(json_decode($data['player_response'])->videoDetails->channelId) ? json_decode($data['player_response'])->videoDetails->channelId : 0;
...
//        if (isset($data['has_cc']) && $data['has_cc'] === 'True')
        ...
        $result['captions_auto_generated'] = isset(json_decode($data['player_response'])->captions->playerCaptionsTracklistRenderer->translationLanguages) ? json_decode($data['player_response'])->captions->playerCaptionsTracklistRenderer->translationLanguages : [];
        ...
        if (isset($data['ps']) && $data['ps'] == 'live')
            $isLive = true;
        if (isset($data['hlsdvr']) && $data['hlsdvr'] == '1')
            $isLive = true;
        if (isset($data['live_playback']) && $data['live_playback'] == '1')
            $isLive = true;
    ...
    protected function getCaptions($videoInfo, $detailed = false) {
        $index = 0;
        $captions = array();
        $videoInfo['caption_tracks'] = isset(json_decode($videoInfo['player_response'])->captions->playerCaptionsTracklistRenderer->captionTracks) ? json_decode($videoInfo['player_response'])->captions->playerCaptionsTracklistRenderer->captionTracks : [];
//        dd($videoInfo['caption_tracks']);
//        $captionTracks = explode(',', $videoInfo['caption_tracks']);
        $captionTracks = $videoInfo['caption_tracks'];
...
EOF;
    ...
    protected function downloadCaption($captions, $language, $filename) {
        if ($language === null)
            $language = $this->defaultCaptionLanguage;
    ...
    public function pathSafeFilename($string) {
        $string = str_replace(
                array_merge(range(chr(0), chr(31)), str_split("#%+&`‘/<>:\"|?*\x5C\x7F")), ' ', trim($string)
        );
    ...
    public function download($itag = null, $resume = false, $caption = false) {
        if ($itag === null && $this->defaultItag !== null)
            $itag = $this->defaultItag;
        if (!$this->isPlaylist) {
            try {
    ...
    public function downloadFull($url, $file, $resume = false) {
        ...
        try {
            $this->downloadFile(
                    $url, $file, function ($downloadSize, $downloaded) use (&$downloadedBytes, &$fileSize, $onProgress, $videosCount) {
    ...
    public function downloadAdaptive($url, $file, $completeSize, $resume = false) {
        ...
        while ($size < $completeSize) {
            try {
                $this->downloadFile(
                        $url . '&range=' . $size . '-' . $completeSize, $file, function ($downloadSize, $downloaded) use (&$downloadedBytes, &$fileSize, $onProgress, $videosCount) {
                    if (!$downloaded && !$downloadSize)
                        return 1;
                    if ($downloadedBytes != $downloaded)
                        $onProgress($downloaded, $downloadSize, $this->currentDownloadingVideoIndex, $videosCount);
